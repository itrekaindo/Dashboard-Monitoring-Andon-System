...
interface ProductSummaryCardLantai1Props {
  data: ProductSummaryLantai1;
}
...
export default function ProductSummaryCardLantai1({ data }: ProductSummaryCardLantai1Props) {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        const res = await fetch(`/api/product/protrack-history?id_product=${encodeURIComponent(String(data.id_product))}&trainset=${encodeURIComponent(String(data.trainset))}&line=${encodeURIComponent(String(data.line ?? 'Lantai 1'))}`);
    ...
    const fetchMissingSubProcess = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        console.error('[ProductSummaryCardLantai1] Error fetching missing subprocess:', e);
