...
interface ProductTrainsetChartLantai1Props {
  data: ProductPercentageLantai1[];
  trainset: string | number;
}

interface GroupedData {
  productName: string;
  items: ProductPercentageLantai1[];
}

export default function ProductTrainsetChartLantai1({ data, trainset }: ProductTrainsetChartLantai1Props) {
  ...
  const groupedData = useMemo(() => {
    ...
    const grouped = new Map<string, ProductPercentageLantai1[]>();
  ...
  const { maxValue, labels, datasets, allItems, itemMap } = useMemo(() => {
    ...
    const nonZeroItems: ProductPercentageLantai1[] = [];
    ...
    const itemMap = new Map<string, Map<string, ProductPercentageLantai1>>();
    nonZeroItems.forEach((item) => {
  ...
  return (
    <Card className="mb-6 border border-gray-700/50 bg-gray-800/50 backdrop-blur-sm">
      <CardHeader className="border-b border-gray-700/50">
        <CardTitle className="text-white">Grafik Produk per Trainset - Lantai 1</CardTitle>
      </CardHeader>
