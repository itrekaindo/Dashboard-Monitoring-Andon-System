...
export default function OperatorClient({ 
  ...
  const [operatorList, setOperatorList] = useState<Array<{nip: number; operator_name: string}>>([]); // For dropdowns
  ...
  const fetchOperatorList = useCallback(async () => {
    try {
      ...
      // Using the operators prop passed to this component
  ...
  // Calculate working days based on selected month filter (excluding weekends and holidays)
  const totalworkdays = (() => {
    let year: number;
    let month: number;
    ...
    if (monthYearFilter === 'all') {
      ...
      // Parse selected month-year filter (format: YYYY-MM)
      const [yearText, monthText] = monthYearFilter.split('-');
      year = Number(yearText);
      month = Number(monthText) - 1; // JavaScript months are 0-indexed
  ...
  // Calculate work hours based on working days (8 hours per day)
  ...
  // Filter and sort jadwal by selected month and search query
  ...
  // Get available month-year options
...
<Card className="bg-gray-800/50 border border-gray-700/60">
 ...
 <div
  ...
  className="p-3 rounded-lg bg-gray-900/60"
>
  ...
  {/* OPERATOR LIST — LANGSUNG NAIK TANPA SPACE */}
  ...
  <div className="text-white font-semibold mt-6 mb-2">
    Pendelegasian Operator
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-900/50 border-b border-gray-700/50">
                  ...
                  <th className="text-left p-4 text-gray-300 font-semibold text-sm">Operator 1{getSortIndicator('operator_assigned1')}</th>
                  <th className="text-left p-4 text-gray-300 font-semibold text-sm">Operator 2{getSortIndicator('operator_assigned2')}</th>
                  <th className="text-left p-4 text-gray-300 font-semibold text-sm">Operator 3{getSortIndicator('operator_assigned3')}</th>
                        ...
                        {isEditing ? (
                          <>
                            ...
                            <td className="p-2 flex gap-2 justify-center">
                              <button
                                onClick={() => {
                                  // Validate: count filled operators should not exceed total_personil
                            ...
                            <td className="p-4 text-center">
                              <button
                                onClick={() => {
                                  ...
                                  // Only populate operators up to total_personil limit
      ...
      <div className="mt-6 flex items-center justify-between text-sm text-gray-400">
        <p>
          Menampilkan {filteredJadwalRows.length} data • Filter bulan: {monthYearFilter === "all" ? "Semua" : monthYearOptions.find(opt => opt.value === monthYearFilter)?.label || monthYearFilter}
        </p>
