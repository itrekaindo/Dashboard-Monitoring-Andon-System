...
import ProductTrainsetChartLantai1 from '@/components/product/ProductTrainsetChartLantai1';
import ProductSummaryListLantai1 from '@/components/product/ProductSummaryListLantai1';
import type { ProductPercentageLantai1 } from '@/lib/queries/production-progress-protrack';

export default function PengerjaanLantai1Page() {
  const [data, setData] = useState<ProductPercentageLantai1[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/product/percentage-lantai1');
  ...
  const getFirstWeekOfMonthTrainset = (items: ProductPercentageLantai1[]): string | null => {
    const now = new Date();
    ...
    <ModernSidebar>
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 p-6">
        <div className="w-full">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-4xl font-bold text-white mb-2">Pengerjaan Lantai 1</h1>
            <p className="text-gray-400">Halaman monitoring pengerjaan produksi lantai 1</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-3 justify-end">
                <label htmlFor="trainset-filter-lantai1" className="text-sm font-medium text-gray-300">
                  Trainset
                </label>
                <select
                  id="trainset-filter-lantai1"
              ...
              {/* Chart */}
              <ProductTrainsetChartLantai1 data={data} trainset={effectiveTrainset} />

              {/* Summary Cards - Only render when effectiveTrainset is set */}
              {effectiveTrainset && <ProductSummaryListLantai1 trainset={effectiveTrainset} />}
            </div>
          ) : (
            <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700/50 rounded-xl p-8">
              <div className="flex items-center justify-center min-h-96">
                <div className="text-center">
                  ...
                  <p className="text-gray-400">
                    Belum ada data pengerjaan lantai 1 yang tersedia
                  </p>
                </div>
              </div>
            </div>
