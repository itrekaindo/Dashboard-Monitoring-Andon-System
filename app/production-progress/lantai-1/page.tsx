...
export default async function Lantai1Page({ searchParams }: { searchParams: Promise<{ [k: string]: string | string[] | undefined }> }) {
  const resolvedSearchParams = await searchParams;
  const lineLabel = "Lantai 1";
  const emptyStats = {
    total_processes: 0,
    completed: 0,
    in_progress: 0,
  ...
  try {
    ...
    console.error("[Lantai1Page] Failed to fetch initial data:", error);
  }
