...
export async function GET(request: Request) {
  try {
    ...
    // Query to extract unique trainsets from id_perproduct for this product
    ...
    const trainsets = rows
      .map((row: any) => row.trainset)
    ...
    console.error('Get trainsets by product error:', error);
    return NextResponse.json(
